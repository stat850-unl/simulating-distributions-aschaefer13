...
#Question 2
exp_density <- ggplot(exp_draw, aes(x = x)) +
  geom_density(color = 'blue') +
  geom_function(fun=dexp, color = 'hotpink', args = list(rate=3))
exp_density
...
mle_gamma <- function(x, a = NULL, b = NULL) {
  ...
  alpha <- fit$estimate['shape'] #chatgpt helped me with this estimate part because I was stuck!
...
set.seed(57391845)
x <- rgamma(100, shape = 3, rate = 2) #I think I got it?
mle_gamma(x)

#Question 2
alphas <- c(0.25, 0.50, 1.0, 2.0, 4.0)
betas <- c(0.25, 0.50, 1.0, 2.0, 4.0)

grid <- expand.grid(alpha = alphas, beta = betas) #got some help on this from the internet

set.seed(57391845)
#I don't think I am doing this correctly
parameters <- mapply(
  function(a, b) rgamma(100, shape = a, rate = b),
  grid$alpha,
  grid$beta
)
true_alpha <- grid$alpha
true_beta <- grid$beta
true_parameters <- apply(parameters, 2, mle_gamma) #I am confused


#Question 3 
#Since I am kind of stuck on question 2, I do not think I can complete question 3. I would guess that at n increases, the error decreases. 
